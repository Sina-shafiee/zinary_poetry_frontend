          ...
          <Gutter>
            <div className={cn('slate-blockToolbarWrapper', 'flex h-[1.5em]')}>
              <div
                className={cn(
                  'slate-blockToolbar',
                  'pointer-events-auto -ms-6 flex items-center',
                )}
              >
...
>(({ children, className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn(
        'slate-gutterRight',
        'absolute top-0 z-50 flex h-full translate-x-full cursor-text hover:opacity-100 sm:opacity-0 main-hover:group-hover:opacity-100',
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        <TooltipPortal>
          <TooltipContent>برای جابجایی بکشید</TooltipContent>
        </TooltipPortal>
      </Tooltip>
    </TooltipProvider>
  );
